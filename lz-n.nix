...
in
{
            ...
            { name, config, ... }:
            {
              ...
              config = {
                lznOpts.before = lib.mkIf (config.loadBefore != [ ]) (
                  lib.concatLines (
                    map (x: "LZN.trigger_load(\"${getName config'.lz-n.specs.${x}.package}\") ") config.loadBefore
                  )
                );
  ...
  config =
    let
      enabledPlugins = lib.filterAttrs (_: value: value.enable) config.lz-n.specs;
    in
    {
      initLua = ''
        LZN = require("lz.n")
        ${lib.concatLines (map (x: "LZN.register_handler(${x})") config.lz-n.handlers)}
        LZN.load("mnw-lz-n")
      '';
      ...
      plugins = {
        start = [
          ...
          (pkgs.stdenvNoCC.mkDerivation {
            ...
            buildCommand = ''
              mkdir -p "$out/lua/mnw-lz-n"
              ${lib.pipe enabledPlugins [
                builtins.attrValues
                (map (
                  x: ''cat ${pkgs.writeText "${x.name}.lua" x.textOutput} > "$out/lua/mnw-lz-n/${x.name}.lua"''
                ))
                lib.concatLines
              ]}
              stylua --indent-type Spaces --indent-width 2 -g "*.lua" "$out/lua/mnw-lz-n" || true
            '';
